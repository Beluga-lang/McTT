...
Inductive gctx_lookup : string -> option exp -> typ -> gctx -> Prop :=
  | ghere : `({{ `#x := [ M ] :: A ∈ Δ, x := [ M ] :: A }})
  | gthere : `(x <> y -> {{ `#y := [ M ] :: A ∈ Δ }} -> {{ `#y := [ M ] :: A ∈ Δ, x := [ N ] :: B }})
...
with wf_ctx_sub : gctx -> ctx -> ctx -> Prop :=
| wf_ctx_sub_empty : 
  `( 
...
| wf_ctx_sub_extend :
  `( {{ Δ ⊢ Γ ⊆ Γ' }} ->
     ...
     {{ Δ ;; Γ ⊢ A ⊆ A' }} ->
     {{ Δ ⊢ Γ, A ⊆ Γ', A' }} )
where "Δ ⊢ Γ ⊆ Γ' " := (wf_ctx_sub Δ Γ Γ') (in custom judg) : type_scope
...
Inductive wf_ctx_eq : gctx -> ctx -> ctx -> Prop :=
| wf_ctx_eq_empty : 
  `( {{ ⊢ Δ }} ->
     {{ Δ ⊢ ⋅ ≈ ⋅ }})
| wf_ctx_eq_extend :
...
#[export]
Instance wf_ctx_eq_Symmetric Δ : Symmetric (wf_ctx_eq Δ).
Proof.
  induction 1; mauto.
Qed.
...
Lemma presup_ctx_sub_gctx : forall {Δ Γ Γ'}, {{ Δ ⊢ Γ ⊆ Γ' }} -> {{ ⊢ Δ }}.
Proof with easy.
